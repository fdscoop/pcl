    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-600 dark:text-slate-400 font-medium">Loading invitations...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">Match Invitations</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Review and respond to match assignments</p>
        </div>
      </div>
      ...
      {filteredInvitations.length === 0 ? (
        <ModernCard className="flex flex-col items-center justify-center py-16 px-6 text-center">
          <div className="p-4 rounded-2xl bg-gradient-to-br from-orange-100 to-amber-100 dark:from-orange-900/30 dark:to-amber-900/30 mb-4">
            <Mail className="h-12 w-12 text-orange-600 dark:text-orange-400" />
          </div>
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">No Invitations</h3>
        ...
        <div className="space-y-4">
          {filteredInvitations.map((invitation) => (
            <ModernCard key={invitation.id} className="p-5 sm:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 py-4 border-y border-slate-100 dark:border-slate-800">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-blue-100 dark:bg-blue-900/30">
                    <Calendar className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                  </div>
                  <div>
                    <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Date</p>
                    <p className="font-medium text-slate-900 dark:text-white">{invitation.match?.match_date ? formatDate(invitation.match.match_date) : 'TBD'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Time</p>
                    <p className="font-medium text-slate-900 dark:text-white">{invitation.match?.match_time ? formatTime(invitation.match.match_time) : 'TBD'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-medium text-slate-400 dark:text-slate-500 uppercase tracking-wide">Venue</p>
                    <p className="font-medium text-slate-900 dark:text-white truncate">{invitation.match?.venue || 'TBD'}</p>
                  </div>
                </div>
              </div>

              {invitation.payout_amount && (
                <div className="mt-4 flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 border border-emerald-200 dark:border-emerald-800">
                  ...
                  <div>
                    <p className="text-sm font-medium text-emerald-600 dark:text-emerald-400">Payout Amount</p>
                    <p className="text-xl font-bold text-emerald-700 dark:text-emerald-300">₹{invitation.payout_amount}</p>
                  </div>
                </div>
              )}
