// ========================================
// Updated Bank Account Verification Component
// Uses separate payout_accounts table with proper verification workflow
// ========================================
...
export function BankAccountVerification({ userId, userData }: BankAccountVerificationProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // NOT verified yet! User will see "Awaiting verification"
      const { data: newAccount, error: insertError } = await supabase
        ...
        .insert({
          ...
          is_primary: payoutAccounts.length === 0 // Primary if first account
        })
        .select()
        .single()
      ...
      if (insertError) {
        ...
        if (insertError.code === '23505') {
          console.error('This bank account is already registered')
        } else {
          console.error(insertError.message || 'Failed to save bank account')
        }
        return
      }

      // Success!
      console.log('Bank account saved successfully! Account is pending verification.')
  ...
  const handleActivateAccount = async (accountId: string) => {
    ...
    try {
      ...
      if (account.verification_status !== 'verified') {
        console.error('Can only activate verified accounts')
        return
      }
      ...
      console.log('Bank account activated successfully')
      await loadPayoutAccounts()
    } catch (error) {
      console.error('Error:', error)
    } finally {
  ...
  const handleDeleteAccount = async (accountId: string) => {
    ...
    try {
      ...
      console.log('Bank account deleted (history preserved)')
      await loadPayoutAccounts()
    } catch (error) {
      console.error('Error:', error)
    } finally {
  ...
  const handleUpdateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Bank account updated! Status reset to pending for re-verification.')
