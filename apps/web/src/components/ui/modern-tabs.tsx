...
const ModernTabs = ({ tabs, activeTab, onChange, variant = 'pills', size = 'md', fullWidth = false }: ModernTabsProps) => {
 ...
 const variants = {
 pills: {
 container: "inline-flex p-1 bg-slate-100 rounded-xl gap-1",
 tab: "rounded-lg transition-all duration-200",
 active: "bg-white text-slate-900 shadow-sm",
 inactive: "text-slate-500 hover:text-slate-700 hover:bg-white/50 "
 },
 underline: {
 container: "inline-flex border-b border-slate-200 gap-1",
 tab: "relative transition-all duration-200 border-b-2 -mb-[1px]",
 active: "text-orange-500 border-orange-500",
 inactive: "text-slate-500 border-transparent hover:text-slate-700 hover:border-slate-300"
 },
 cards: {
 ...
 inactive: "bg-white border-slate-200 text-slate-600 hover:border-orange-300 "
 },
 segment: {
 container: "inline-flex p-1 bg-slate-100 rounded-xl",
 tab: "rounded-lg transition-all duration-200",
...
const ScrollableTabs = ({ tabs, activeTab, onChange }: ScrollableTabsProps) => {
 ...
 >
 <div className="inline-flex p-1 bg-slate-100 rounded-xl gap-1 min-w-max">
 {tabs.map((tab) => (
 <button
 key={tab.id}
