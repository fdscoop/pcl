...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'contracts') THEN
        ...
        -- Create partial unique index for single active contract
        CREATE UNIQUE INDEX single_active_contract ON contracts(player_id, club_id, status) WHERE status = 'active';
